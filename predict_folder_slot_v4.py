#!/usr/bin/env python3
"""predict_folder_slot_v4 — v4 드라이버 (v3 + 약한 시각 tie-break: bg하이라이트 + 텍스트대비).

full OCR → slot_v4.rolling_analyze(롤링 FIFO + 박스분할 + 강한 선택) → per_frame.csv + 정확도.
...
def main():
    ...
    ap.add_argument("--bg-window", type=int, default=5, help="배경/대비 힌트 누적 프레임 수(약한 tie-break)")
    ap.add_argument("--bg-weight", type=float, default=0.3, help="bg 하이라이트 tie-break 보너스")
    ap.add_argument("--bg-thresh", type=float, default=12.0, help="bg 하이라이트 발동 임계(색거리)")
    ap.add_argument("--contrast-weight", type=float, default=0.2, help="텍스트-배경 대비 tie-break 보너스")
    ap.add_argument("--contrast-thresh", type=float, default=25.0, help="대비 발동 임계")
    ...
    for g, uids in sorted(seqs.items()):
        ...
        if not frames:
            continue
        primary = V4.rolling_analyze(frames, ok, window=args.window, by_height=args.by_height,
                                     band=args.band, conf_thr=args.min_conf,
                                     bg_window=args.bg_window, bg_weight=args.bg_weight,
                                     bg_thresh=args.bg_thresh, contrast_weight=args.contrast_weight,
